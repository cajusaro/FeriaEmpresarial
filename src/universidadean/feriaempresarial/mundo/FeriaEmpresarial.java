...
public class FeriaEmpresarial {
    ...
    /**
     * Porcentaje punto de equilibrio
     */
    public static final int PORCENTAJE_PUNTO_EQUILIBRIO = 60;

    /**
     * Punto de equilibrio no cumplido
     */
    public static final String PUNTO_EQUILIBRIO_NO_CUMPLIDO = "El punto de equilibrio no se ha cumplido";

    /**
     * Punto de equilibrio cumplido
     */
    public static final String PUNTO_EQUILIBRIO_CUMPLIDO = "El punto de equilibrio se ha cumplido";

    /**
     * Tipo de empresa expositora
     */
    public static final String EXPOSITORA = "Expositora";
    ...
    public String metodo1() {
        ...
        String currentMaxZona           = "";
        int currentMaxValuevalue        = 0;
        HashMap<String, Integer> map    = new HashMap<String, Integer>();

        for (int i = 0; i < puestos.length; i++) {  //Se hace un for para todos los puestos

            Puesto puesto = puestos[i];
            if (puesto.estaOcupado()){
                String sEmpresaExpositora = puesto.darNombreEmpresa();
                String sZona = puesto.darZona();

                for (int j = 0; j < empresas.size(); j++) {  //for para sumar cantidades de todas las zonas
                    Empresa empresa = empresas.get(j);
                    if (empresa.darNombre().equals(sEmpresaExpositora) && empresa.darTipo().equals(EXPOSITORA)) {
                        int iAsistentes = empresa.darNumeroPersonasAsistentes();
                        int count = map.containsKey(sZona) ? map.get(sZona) : 0;
                        map.put(sZona, count + iAsistentes);
                        break;
                    }
                }
            }
        }
        for (Map.Entry<String, Integer> entry : map.entrySet()){  //Selecciona la zona con cantidad maxima
            if (entry.getValue() > currentMaxValuevalue){
                currentMaxZona = entry.getKey();
                currentMaxValuevalue = entry.getValue();
            }
        }
        return currentMaxZona + "=" + currentMaxValuevalue + " Expositores";
    }
    ...
    public String metodo2() {
        ...
        String sRetorno = PUNTO_EQUILIBRIO_NO_CUMPLIDO;

        double dPorcentajeOcupacion = porcentajeOcupacion();

        if (dPorcentajeOcupacion >= PORCENTAJE_PUNTO_EQUILIBRIO) sRetorno = PUNTO_EQUILIBRIO_CUMPLIDO;
